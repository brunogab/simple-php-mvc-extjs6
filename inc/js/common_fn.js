...
function trycatchex(ex) {
	Ext.MessageBox.show({
		title: 'ERROR Exception (trycatchex)',
		cls: 'msgBox',
		buttons: Ext.Msg.OK,
		icon: Ext.Msg.ERROR,
		msg: 'Exception Error description: ' + ex.toString()
	});
	return;
}
...
function failure_section_Form(form, action) {
	...
	if (action.failureType === Ext.form.action.Action.CLIENT_INVALID) {
		Ext.MessageBox.show({
			title: 'CLIENT_INVALID',
			cls: 'msgBox',
			buttons: Ext.Msg.OK,
			icon: Ext.Msg.ERROR,
			msg: 'Something has been missed. Please check and try again.'
		});
		return;
	}
	/**
	...
	if (typeof action.result.info === 'undefined') {
		Ext.MessageBox.show({
			...
			buttons: Ext.Msg.OK,
			icon: Ext.Msg.ERROR,
			msg: 'action.result.info = undefined'
		});
		return;
	}
}
...
function failure_section_Grid(proxy, response, options) {
	if (response.statusText != 'OK') {
		//timeout or Communication failed
		Ext.MessageBox.show({
			title: 'CONNECT_FAILURE',
			cls: 'msgBox',
			buttons: Ext.Msg.OK,
...
function noDirty() {
	extToast(
		Ext.String.format('{0}', 'Info'),
		Ext.String.format('{0}', 'Data is not dirty.'),
		'info'
	);
	return;
}
function noValid() {
	extToast(
		Ext.String.format('{0}', 'Info'),
		Ext.String.format('{0}', 'Data is not valid'),
		'info'
	);
	return;
}
