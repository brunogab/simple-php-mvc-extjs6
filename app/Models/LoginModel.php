...
final class LoginModel
{
	...
	{   
		$query_result = $this->db->sql_query("SELECT T0.id AS 'id', T0.Ucode AS 'ucode', T0.Uname AS 'uname', T0.Email AS 'email',
							...
							FROM users T0 WHERE LOWER(T0.Ucode) collate utf8mb4_bin = LOWER(:ucode) collate utf8mb4_bin",  array(':ucode' => $ucode));
		if (!$query_result) {
			return false;
		}
	...
	{
		$query_result = $this->db->sql_query("UPDATE users SET wrongLogin = wrongLogin + 1 WHERE LOWER(Ucode) collate utf8mb4_bin = LOWER(:ucode) collate utf8mb4_bin", array(':ucode' => $ucode));
		if (!$query_result) {
			return false;
		}
		return $query_result;
	...
	{
		$query_result = $this->db->sql_query("UPDATE users SET wrongLogin = 0 WHERE LOWER(Ucode) collate utf8mb4_bin = LOWER(:ucode) collate utf8mb4_bin", array(':ucode' => $ucode));
		if (!$query_result) {
			return false;
		}
		return $query_result;
