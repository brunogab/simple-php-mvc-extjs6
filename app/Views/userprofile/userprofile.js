...
Ext.define('uprv_Model', {
	...
	proxy: {
		...
		afterRequest: function(request, success) {
			try {
				if (request._action == 'read') {
					if (
						uprv_Store.getCount() < 1 &&
						Ext.getCmp('uprv_searchfield').getValue() != ''
					) {
						extToast(
							Ext.String.format('{0}', text_title_warning),
							Ext.String.format('{0}', text_search_empty),
							'warning'
						);
					}
				}
		...
		listeners: {
			//ERROR or http failure or relogin or nopriv come with success = false
