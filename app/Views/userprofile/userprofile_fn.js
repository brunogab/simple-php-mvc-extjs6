		...
		success: function (form, action) { //success = true
			try {
				...
				if (action.result.info!=''){
					extToast(Ext.String.format('{0}', 'Success'), Ext.String.format('{0}', action.result.info), 'success');
				}
				extToast(Ext.String.format('{0}', 'Success'), Ext.String.format('{0}', 'Data has been saved.'), 'success');
			}
			catch (ex) {trycatchex(ex);}
		},
		failure: function (form, action) {
